...
| `colonstop.py` | Whether the running server ends an agent turn it meant to continue — replays real stalls from the gateway's text-level trace with byte-identical prompts and counts how often the turn ends again. Starts no server: an A/B of two Halogen configurations is the operator's restart, because `sideserver.py` cannot start a Halogen container (how 23.09. did it: `reports/2026-09-23_colonstop/README.md`). `--strip-stalls` replays without the earlier stall episodes, `--announcements` replays turns that DID go on — the counter-sample: stalls picked where one checkpoint stalled favour the other (`reports/2026-09-24_halogen-0.13.8/README.md`) |
